...
/**
 * A lock-free queue for single consumer, single producer. Not thread-safe when using
 * multiple consumers or producers.
 *
 * Example code:
 *
 ...
 * the maximum capacity. Included for ease of units testing because testing queue lengths of
...
class LockFreeQueue {
     ...
     *
     * IMPORTANT: This implementation is only thread-safe with a single reader thread and single
     * writer thread. Have more than one of either will result in bad things.
     */
public:
    static constexpr bool isPowerOfTwo(int32_t n) {
        return (n & (n - 1)) == 0;
    }
    ...
    INDEX_TYPE size() const {
         ...
         *
         * Whilst writeCounter is greater than readerCounter the result of (write - read) will always
         * be positive. Simple.
         *
