...
/**
 * Main audio engine for the Project. It is responsible for:
 *
 * - Creating a callback object which is supplied when constructing the audio stream, and will
 * be called when the stream starts.
...
void HelloOboeEngine::setBufferSizeInBursts(int32_t numBursts) {
    ...
    if (result) {
        LOGD("Buffer size succuessfully changed to %d", result.value());
    } else {
...
double HelloOboeEngine::getCurrentOutputLatencyMillis() {
    ...
    if (result == oboe::Result::OK) {
        ...
        int64_t nextFrameWriteTime = duration_cast<nanoseconds>(
