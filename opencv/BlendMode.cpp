...
static cv::Mat& layerModelBlending(const cv::Mat& src, const cv::Mat& blendLayer,
                                   cv::Mat& dst, int blendType) {
            ...
            for (int index = 0; index < 3; index++) {
                switch (blendType) {
                    case DARKEN: {
                        ...
                        dst.at<cv::Vec3f>(row, col)[index] = std::min(
                                src.at<cv::Vec3f>(row, col)[index],
                                blendLayer.at<cv::Vec3f>(row, col)[index]);
                        break;
                    ...
                    case LIGHTEN: {
                        ...
                        dst.at<cv::Vec3f>(row, col)[index] = std::max(
                                src.at<cv::Vec3f>(row, col)[index],
                                blendLayer.at<cv::Vec3f>(row, col)[index]);
                        break;
                    }
...
int main(int argc, char** argv) {
    ...
    if (argc == 4) {
        ...
        printf("Usage: %s input1 input2 (type)\n", argv[0]);
        exit(EXIT_FAILURE);
    }

    cv::Mat src = cv::imread(argv[1]);
    cv::Mat blendLayer = cv::imread(argv[2]);
    cv::Mat dst(src.size(), CV_8UC3, cv::Scalar::all(0));

    layerModelBlending(src, blendLayer, dst, type);
