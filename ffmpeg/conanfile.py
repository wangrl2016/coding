...
编译该脚本 conan create conan_file_dir
例如conanfile.py在当前路径 conan create .
...
class FFMpegConan(ConanFile):
    ...
    def build_configure(self):
        with tools.chdir(self._source_subfolder):
            ...
            args.append('--disable-avfilter')
            if self.settings.build_type == 'Debug' and self.settings.os != "iOS":
            ...
            try:
                ...
                if self.settings.os == 'iOS':
                    ...
                    # args.append('--sysroot=%s' % xcrun.sdk_path)
                    args.append('--target-os=darwin')
                    args.append('--arch=%s' % tools.to_apple_arch(self.settings.arch))
                    # args.append('--disable-debug')
                    if self.settings.arch == 'armv7':
                        args.append('--as=/usr/local/bin/gas-preprocessor.pl -- xcrun -sdk iphoneos clang')
                    elif self.settings.arch == 'armv8':
                        args.append(
                            '--as=/usr/local/bin/gas-preprocessor.pl -arch aarch64 -- xcrun -sdk iphoneos clang')
                    elif self.settings.arch == 'x86_64':
                        args.append(
                            '--as=/usr/local/bin/gas-preprocessor.pl -arch aarch64 -- xcrun -sdk iphonesimulator clang')
                        # args.append('--cc=xcrun -sdk iphoneos clang')
                    # args.append('--cxx=%s' % xcrun.cxx)
                    # args.append('--ar=%s' % xcrun.ar)
                    # args.append('--extra-cflags=-arch %s -mios-version-min=10.0' % tools.to_apple_arch(self.settings.arch))
                    # args.append('--extra-ldflags=-arch %s -mios-version-min=10.0' % tools.to_apple_arch(self.settings.arch))
    ...
    def package_info(self):
        libs = ['avformat', 'avcodec', 'swresample', 'swscale', 'avutil']
