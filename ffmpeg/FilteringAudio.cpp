...
static const char* FILTERS_DESCR = "aresample=8000,aformat=sample_fmts=s16:channel_layouts=mono";
...
static int open_input_file(const char* filename) {
    ...
    ret = av_find_best_stream(formatContext, AVMEDIA_TYPE_AUDIO, -1, -1, &dec, 0);
    if (ret < 0) {
    ...
    decodeContext = avcodec_alloc_context3(dec);
    if (!decodeContext)
        return AVERROR(ENOMEM);
    avcodec_parameters_to_context(decodeContext, formatContext->streams[audioStreamIndex]->codecpar);
    /* init the audio decoder */
    if ((ret = avcodec_open2(decodeContext, dec, nullptr)) < 0) {
...
static int init_filters(const char* filters_descr) {
    ...
    AVRational time_base = formatContext->streams[audioStreamIndex]->time_base;
    ...
    ret = av_opt_set_int_list(bufferSinkContext, "channel_layouts", out_channel_layouts, -1,
                              AV_OPT_SEARCH_CHILDREN);
    if (ret < 0) {
...
int main(int argc, char** argv) {
    ...
    if ((ret = open_input_file(argv[1])) < 0)
        goto end;
    if ((ret = init_filters(FILTERS_DESCR)) < 0)
        goto end;
    /* read all packets */
    while (1) {
