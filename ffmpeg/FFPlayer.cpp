...
static int queue_picture(VideoState* is, AVFrame* src_frame, double pts,
                         double dur, int64_t pos, int serial) {
    Frame* vp;
...
static int configure_audio_filters(VideoState* is, const char* afilters, int force_output_format) {
    ...
    if (is->audio_filter_src.channel_layout)
        snprintf(asrc_args + ret, sizeof(asrc_args) - ret,
                 ":channel_layout=0x%ld", is->audio_filter_src.channel_layout);
        ...
        if (got_frame) {
            ...
            if (reconfigure) {
                ...
                av_log(nullptr, AV_LOG_DEBUG,
                       ...
                       frame->sample_rate, frame->channels, av_get_sample_fmt_name((AVSampleFormat) frame->format),
                       buf2,
...
static int video_thread(void* arg) {
    ...
    for (;;) {
        ...
        if (last_w != frame->width
            ...
            if ((ret = configure_video_filters(graph, is, vfilters_list ? vfilters_list[is->vfilter_idx] : nullptr,
                                               frame)) < 0) {
                SDL_Event event;
...
static DEFINE_string_s(input, i, "", "Input movie file");

int main(int argc, char** argv) {
    CommandLineFlags::SetUsage("Simple media player based on the FFmpeg libraries");
